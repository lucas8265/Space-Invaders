...
function displayGameOver(){
  ...
  var existingHighScores = JSON.parse(window.localStorage.getItem('high_scores') || '[]');
  existingHighScores.push(highScoreItem);
  // sort by highest score
  existingHighScores = existingHighScores.sort(function(scoreDatum1, scoreDatum2) {
    return scoreDatum2.score - scoreDatum1.score;
  });

  window.localStorage.setItem('high_scores', JSON.stringify(existingHighScores));
  for (var i = 0; i < existingHighScores.length; i++) {
    var text = `Name: ${existingHighScores[i].name} Score: ${existingHighScores[i].score}`

    Crafty.e("GameOver, 2D, Canvas, Text")
      .attr({ x: 75, y: 60 + 30 * i, z: 20 })
      .text(text).textColor('#FFFF00').textFont({ size: '18px', family: 'joystixmonospace' });
  }
